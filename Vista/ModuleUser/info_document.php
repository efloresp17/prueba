 <?php
    ...
    if ($t_documento->getCod_t_documento() == 1) {
        $variable = ManejoBook::consultBook($documento);
        ...
        $variable = ManejoPresentation::consultTitle($documento);
    }
    $estado = ManejoState::consult($listDoc->getCod_estado());
    $estado_reserva = ManejoReservation_Status::consult($listDoc->getCod_estado_reserva());
    $verificar = ManejoReservation::consultReservationDocumentStatus($documento, $codigoUsuario);
    $status = ManejoReservation::getListStatusAct($codigoUsuario);
    //echo $verificar->getCod_documento();
    //echo $documento;
    //echo $verificar->getEstado();
    //echo $verificar->getCod_reserva();
    ?>
 <!-- Start main Content -->
                         ...
                         <div class="col-lg-6 col-12">
                             <div class="product__info__main">
                                 ...
                                 <div class="product-info-stock-sku d-flex">
                                     ...
                                     <p>Reservation status: <span> <?php echo $estado_reserva->getNom_estado_reserva(); ?></span></p>
                                 </div>
                                 <!--<div class="product-info-stock-sku d-flex">
                                     <p>Autor: <span></span></p>
                                 </div>-->
                                 ...
                                 <div class="box-tocart d-flex">
                                     <div class="addtocart__actions">
                                         <!--AQUI-->
                                         <?php
                                            if ($listDoc->getCantidad() == 0) {
                                                echo '';
                                            } else {
                                                if ($estado_reserva->getCod_estado_reserva() == 2 && $estado->getCod_estado() == 1) {
                                                    //echo '<button class="tocart" type="submit" title="Add to Cart">Reserve</button>'
                                                    //echo $verificar->getCod_usuario();
                                                    echo '<form action="ModuleUser/actionReserve.php?codDoc=' . $documento . '&action=reserved" method="post">';
                                                    echo '<input ';
                                                    echo ' type="submit" class="tocart" name="reserve" value="Reserve" ';

                                                    if ($verificar->getCod_documento() == $documento && $verificar->getEstado() == 'Activated') {
                                                        echo 'title="Not Reserve" disabled';
                                                    } else {
                                                        echo 'title="Reserve"';
                                                    };
                                                    echo '></input>';
